...
let seqadd_test3 _test_ctxt =
  (* Testing sequence add on 1 digit numbers *)
  assert_equal [9;3;8] (seqadd [3;1;7] [6;2;1]);;

(* another test of seqadd on two non-empty lists *)
let seqadd_test4 _test_ctxt =
  (* Testing sequence add on 2 digit numbers*)
  assert_equal [20;5;6] (seqadd [14;3;5] [6;2;1]);;

(* test of seqmult on two empty lists *)
let seqmult_test1 _test_ctxt =
  (* Testing sequence mult for single digits *)
  assert_equal [8;6;5] (seqmult [2;3;5] [4;2;1]);;

(* test seqmult on two non-empty lists *)
let seqmult_test2 _test_ctxt =
  (* Testing sequence mult for double digit answer *)
  assert_equal [12;2;10] (seqmult [3;1;5] [4;2;2]);;

(* test seqmult on two non-empty lists *)
let seqmult_test3 _test_ctxt =
  (* Testing sequence mult for double digit numbers *)
  assert_equal [40;12;14] (seqmult [10;1;7] [4;12;2]);;

(* test seqmult on two non-empty lists *)
let seqmult_test4 _test_ctxt =
  (* Testing sequence mult for triple digit answers *)
  assert_equal [48;121;21] (seqmult [12;11;7] [4;11;3]);;
...
let seqmult_zeros =
  ...
  QCheck.Test.make ~name:"seqmult_zeros" ~count:1000
    ...
    (fun (seq,zeros) -> seqmult [1;2;3] [0;0;0] = [0;0;0];
     sum [0;0;0] = 0);;
